...
import {
  DomSanitizer
} from "./chunk-EDDRHLVA.js";
import {
  HttpClient
} from "./chunk-LGQMN2ME.js";
import {
  DOCUMENT,
  isPlatformBrowser
} from "./chunk-RTWQK3RW.js";
import {
...
var MatIconRegistry = class _MatIconRegistry {
  ...
  getSvgIconFromUrl(safeUrl) {
    ...
    if (cachedIcon) {
      return (0, import_rxjs6.of)(cloneSvg(cachedIcon));
    }
    return this._loadSvgIconFromConfig(new SvgIconConfig(safeUrl, null)).pipe((0, import_operators5.tap)((svg) => this._cachedIconsByUrl.set(url, svg)), (0, import_operators5.map)((svg) => cloneSvg(svg)));
  }
  /**
  ...
  getNamedSvgIcon(name, namespace = "") {
    ...
    return (0, import_rxjs6.throwError)(getMatIconNameNotFoundError(key));
  }
  ngOnDestroy() {
    this._resolvers = [];
  ...
  _getSvgFromConfig(config) {
    if (config.svgText) {
      return (0, import_rxjs6.of)(cloneSvg(this._svgElementFromConfig(config)));
    } else {
      return this._loadSvgIconFromConfig(config).pipe((0, import_operators5.map)((svg) => cloneSvg(svg)));
    }
  }
  /**
  ...
  _getSvgFromIconSetConfigs(name, iconSetConfigs) {
    ...
    if (namedIcon) {
      return (0, import_rxjs6.of)(namedIcon);
    }
    const iconSetFetchRequests = iconSetConfigs.filter((iconSetConfig) => !iconSetConfig.svgText).map((iconSetConfig) => {
      return this._loadSvgIconSetFromConfig(iconSetConfig).pipe((0, import_operators5.catchError)((err) => {
        const url = this._sanitizer.sanitize(SecurityContext.RESOURCE_URL, iconSetConfig.url);
        const errorMessage = `Loading icon set URL: ${url} failed: ${err.message}`;
        this._errorHandler.handleError(new Error(errorMessage));
        return (0, import_rxjs6.of)(null);
      }));
    });
    return (0, import_rxjs6.forkJoin)(iconSetFetchRequests).pipe((0, import_operators5.map)(() => {
  ...
  _loadSvgIconFromConfig(config) {
    return this._fetchIcon(config).pipe((0, import_operators5.tap)((svgText) => config.svgText = svgText), (0, import_operators5.map)(() => this._svgElementFromConfig(config)));
  }
  /**
  ...
  _loadSvgIconSetFromConfig(config) {
    ...
    return this._fetchIcon(config).pipe((0, import_operators5.tap)((svgText) => config.svgText = svgText));
  }
  /**
  ...
  _fetchIcon(iconConfig) {
    ...
    const req = this._httpClient.get(url, {
      responseType: "text",
      withCredentials
    }).pipe((0, import_operators5.map)((svg) => {
      return trustedHTMLFromString(svg);
    }), (0, import_operators5.finalize)(() => this._inProgressUrlFetches.delete(url)), (0, import_operators5.share)());
...
var MatIcon = class _MatIcon {
  ...
  constructor(_elementRef, _iconRegistry, ariaHidden, _location, _errorHandler, defaults) {
    ...
    this._currentIconFetch = import_rxjs6.Subscription.EMPTY;
    if (defaults) {
      if (defaults.color) {
  ...
  _updateSvgIcon(rawName) {
    ...
    if (rawName) {
      ...
      this._currentIconFetch = this._iconRegistry.getNamedSvgIcon(iconName, namespace).pipe((0, import_operators5.take)(1)).subscribe((svg) => this._setSvgElement(svg), (err) => {
